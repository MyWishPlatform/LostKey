...
contract('LostKeyWallet', function (accounts) {
    ...
    it('#8 cannot check before time', async () => {
        const contract = await LostKeyWallet.new(TARGET, [RECIPIENT_1, RECIPIENT_2], [50, 50], 10 * MINUTE);
        await contract.sendTransaction({ value: web3.toWei(1, 'ether') });
    ...
    it('#10 successfully sent funds after check', async () => {
        ...
        const contract = await LostKeyWallet.new(TARGET, recipients, percents, 10 * MINUTE);
    ...
    it('#11 cannot set not 100 percents', async () => {
        await LostKeyWallet.new(TARGET, [TARGET], [90], MINUTE).should.eventually.be.rejected;
        await LostKeyWallet.new(TARGET, [TARGET], [110], MINUTE).should.eventually.be.rejected;
    });
});
